...
1. Создайте `.env` на основе шаблона и заполните переменные:

```bash
cp faxmsgr.Server/.env.prod.example .env
# Отредактируйте .env — заполните пароли и секреты
```

2. Запустите все сервисы из корня репозитория:
