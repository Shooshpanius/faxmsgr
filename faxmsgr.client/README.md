...
|---|---|---|
| `VITE_API_URL` | URL Go-сервера для проксирования в dev-режиме (по умолчанию `http://localhost:8080`) | Нет |
| `VITE_YM_COUNTER_ID` | Номер счётчика Яндекс Метрики | Нет |

Если `VITE_YM_COUNTER_ID` не задан, Яндекс Метрика не подключается.
